...
static int buddy_memalign(void **ptr, size_t alignment, size_t size) {
#ifdef _WIN32
...
static void buddy_memfree(void *ptr) {
#ifdef _WIN32
    _aligned_free(ptr);
#else
    free(ptr);
...
buddy_allocator_t *buddy_create(size_t capacity_bytes) {
    ...
    int rc = buddy_memalign((void **)&allocator->base, BUDDY_MIN_BLOCK_SIZE, allocator->capacity);
    if (rc != 0) {
